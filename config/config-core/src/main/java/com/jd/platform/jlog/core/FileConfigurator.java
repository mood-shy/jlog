...
import static com.jd.platform.jlog.core.FileConstant.*;
...
public class FileConfigurator implements Configurator {
    ...
    private void checkAndConfigure(){
        ...
        FILE_MODIFY_MAP.forEach((k, v)->{
            ...
            if(newFile != null && newFile.change){
                boolean same = CollectionUtil.equals(newFile.props, v.props);
                if(!same){
                    LOGGER.warn("文件配置变更:{}", k, JSON.toJSONString(newFile.props));
                    change.set(true);
    ...
    private Map<String, FileWrapper> checkAndReload() {
        ...
        System.out.println("定时任务checkAndReload: "+JSON.toJSONString(LISTENED_FILES));
        for (String fileName : LISTENED_FILES) {
            ...
            LOGGER.debug("定时扫描fileName:{} 更新了 最新时间:{}  缓存时间:{}  threadName ={}",fileName,longToDate(curLastMod), longToDate(cacheLastMod),Thread.currentThread().getName());
            ...
            try (InputStream is = url.openStream()) {
                ...
                LOGGER.debug("定时任务：{}配置文件配置:{}", fileName, JSON.toJSONString(props));
            } catch (Exception e) {
