...
* 启动worker
   * 配置ck地址，从config模块引入配置器[apollo.zk..，如serverAddr=127.0.0.1:2181；参考example项目application.properties
...
* 启动web服务（client端）参照example项目
   ...
   *  在配置文件application.yml/application.propertires/jLog.propertires/其他配置中心 配置压缩提取等配置 
