...
* 使用docker安装clickhouse（docker安装参考：https://www.runoob.com/docker/docker-tutorial.html）
   * 拉取镜像：docker pull yandex/clickhouse-server
   * 拉取镜像：docker pull yandex/clickhouse-client
   ...
   * 建表：[sql](https://gitee.com/jd-platform-opensource/jlog/blob/master/worker/src/main/resources/jlog.sql)

* 启动worker
   * 配置ck地址，从config模块选择配置中心[apollo.zk.etcd.nacos]，
               并在application.yml/propertires文件 ，如serverAddr=127.0.0.1:2181；参考example项目application.properties
               如无第三方配置中心则默认file配置器, （生产环境）建议继承FileConfigurator复写以维护worker配置，参考CustomConfigurator.class    
   * 启动worker，开启接收data
   * 自动上报地址到配置中心
   * 内部使用Disruptor并行处理存储
...
* 启动web服务（client端）参照example项目
   ...
   *  在配置文件（选一个）application.yml/application.propertires/jLog.propertires/其他配置中心 配置压缩提取等配置 
                      参考example项目application.properties
   *  如果未配置workers，则向本地发送数据（用于测试）
   *  如有需要可自定义额外的出参入参提取，实现RespMap/ReqMap即可，参照CustomRespMap.class
